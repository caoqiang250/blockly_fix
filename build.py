...
# build.py with no parameters builds all files.  没有执行参数表示压缩所有文件
# core builds blockly_compressed, blockly_uncompressed, and blocks_compressed.  参数core表示压缩标准核心组件
# accessible builds blockly_accessible_compressed,  参数accessible表示压缩视力损坏人员核心组件 
#  blockly_accessible_uncompressed, and blocks_compressed.
# generators builds every <language>_compressed.js. 参数generators表示压缩转换编程语言组件
# langfiles builds every msg/js/<LANG>.js file. 参数langfiles表示压缩本地化语言组件

# This script generates four versions of Blockly's core files. The first pair
# are:
# 正常压缩
#   blockly_compressed.js 完全压缩
#   blockly_uncompressed.js 不完全压缩，依次加载核心JS
...
class Gen_compressed(threading.Thread):
  ...
  def do_compile(self, params, target_filename, filenames, remove):
    ...
    conn = httplib.HTTPSConnection("127.0.0.1",1080)
    conn.request("POST", "https://closure-compiler.appspot.com/compile", urllib.urlencode(params), headers)
